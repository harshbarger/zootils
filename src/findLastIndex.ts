...
function _findLastIndex<I extends any[]>(
  data: I,
 ...
 * @category Array
 */
export function findLastIndex<I extends any[]>(
  data: I,
  condition: Predicate<ElementType<I>>
): number;
...
export function findLastIndex(...args: any[]) {
  return purry(_findLastIndex, ...args);
}
...
if (import.meta.vitest) {
  ...
  it("returns the index when a passsing element exsits", () => {
    expect(findLastIndex([2, -10, -2, 4], (x) => x < 0)).toBe(2);
    expect(findLastIndex((x) => x < 0)([2, -10, -2, 4])).toBe(2);
  });

  it("returns NaN index when no passsing element exsits", () => {
    expect(findLastIndex([2, 10, 12, 4], (x) => x < 0)).toBe(NaN);
  });
}
